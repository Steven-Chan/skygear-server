...
type AuthInfo []skydb.AuthInfo

func (a AuthInfo) Len() int           { return len(a) }
func (a AuthInfo) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a AuthInfo) Less(i, j int) bool { return a[i].ID < a[j].ID }
...
func (conn *testRelationConn) QueryRelation(user string, name string, direction string, config skydb.QueryConfig) []skydb.AuthInfo {
	...
	sort.Sort(AuthInfo(conn.AuthInfo))
	if config.Limit == 0 {

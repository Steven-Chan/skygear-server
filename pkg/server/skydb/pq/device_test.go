...
func TestDevice(t *testing.T) {
	Convey("Conn", t, func() {
		...
		Convey("creates a new Device", func() {
			...
			err = c.QueryRowx("SELECT type, token, topic, auth_id, last_registered_at FROM _device WHERE id = 'deviceid'").Scan(&deviceType, &token, &topic, &userInfoID, &lastRegisteredAt)
			So(err, ShouldBeNil)
